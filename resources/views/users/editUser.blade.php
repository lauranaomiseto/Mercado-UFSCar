<x-layout>
    ...
    <form action="{{ route('users.update', ['user' => $user->id]) }}" method="POST" class="w-fit m-auto">
        ...
        @enderror
        <x-inputs.basic type="text" label="Nome:" name="name" value="{{ $user->name }}" placeholder="usuario@exemplo.com">
        </x-inputs>

        @error('email')
            <div class="text-red-300 max-w-[295px]">{{ $message }}</div>
        @enderror
        <x-inputs.basic type="text" label="Email:" name="email" value="{{ $user->email }}" placeholder="usuario@exemplo.com">
        </x-inputs>
